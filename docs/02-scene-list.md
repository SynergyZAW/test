# 02 — Scene list and storyboard

Progress `p` runs 0 to 1 across the pinned film (chapters 1–6, a 1000vh scroll track). Chapters 7–8 are normal flow.
Every number below is a creative decision and is live in `src/data/chapters.ts` and `src/lib/scene.ts`. Change with intent.
...
## Scroll and load budget

- Film: 1000vh scroll track, six chapters, ~750 frames at 12 frames per scroll-second. Portrait master at 810×1440 WebP q70 (~18KB) ≈ 13MB total, loaded per chapter, chapter 1 preloaded, poster per chapter as the stills fallback. Landscape at 1440×810 for viewports wider than 1.1:1.
- Camp and afterglow: static images and SVG.
- Fonts: Badaboom subset 10KB, Atkinson two weights 23KB.
