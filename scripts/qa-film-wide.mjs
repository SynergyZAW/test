...
for (const p of [0.05, 0.3, 0.46, 0.55, 0.75, 0.95]) {
  ...
  await page.screenshot({ path: `test-results/film/wide-${i++}.png` })
}
console.log(await page.evaluate(() => performance.getEntriesByType('resource').filter((r) => r.name.includes('/film')).map((r) => r.name.includes('film-wide') ? 'wide' : 'portrait').filter((n, i, a) => a.indexOf(n) === i).join(',')))
await browser.close()
